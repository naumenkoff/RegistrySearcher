...
public static class Program
{
    ...
    private static void StartSearch()
    {
        ...
        var excludedKeys =
            GetUserVirtualInput($"Введите нежелательные для поиска разделы реестра, сейчас их {BlackFilter.Count} > ",
                ConsoleColor.Red);
        AddExcludedKeys(excludedKeys);
        PrintElapsedTime(registrySearcher);
        var resultMessage = registrySearcher.StartSearch().Result;
    ...
    private static void CheckHistory()
    {
        ...
        if (GetUserPhysicalInput("Если вы хотите просмотреть историю найденных совпадений, нажмите Y > ", ConsoleKey.Y,
                ConsoleColor.Red) is false) return;
        foreach (var workResult in RegistrySearcher.History) PrintColoredLine(workResult, ConsoleColor.Green);
    }

    private static void ExitOrContinueExecution()
    {
        if (GetUserPhysicalInput(
                ...
                ConsoleKey.Enter, ConsoleColor.Red)) return;
        Main();
    }
    ...
    private static void SaveResultInFile(RegistrySearcher rs)
    {
        if (GetUserPhysicalInput("Если вы хотите сохранить полученный результат на рабочий стол, нажмите Y > ",
                ConsoleKey.Y, ConsoleColor.Red) is false) return;
        var result = rs.SaveResult().Result;
