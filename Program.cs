...
public static class Program
{
    ...
    private static void StartSearch()
    {
        ...
        var multithreaded =
            GetUserPhysicalInput("Нажмите Y чтобы поиск был многопоточным, иначе он будет однопоточным > ",
                ConsoleKey.Y, ConsoleColor.Red);
        var excludedKeys =
        ...
        var resultMessage = registrySearcher.DoWork(multithreaded
            ? RegistrySearcher.SearchType.MultiThread
            : RegistrySearcher.SearchType.SingleThread);
    ...
    private static void ExitOrContinueExectuion()
    {
        if (GetUserPhysicalInput(
                ...
                ConsoleKey.Enter, ConsoleColor.Red) is true) return;
        Main();
    }
